...
/*            else
    ...
    public Vector3 getNodeRelativePosition(int modelTargetID)
    {
        GameObject modelTarget;
        modelTargets.TryGetValue(modelTargetID, out modelTarget);
        ...
        return model.getOutlineRotationInv() * (trackedPosition - originPosition);
    }

    public float getNodeRelativeRotation(int modelTargetID)
    {
        GameObject modelTarget;
        modelTargets.TryGetValue(modelTargetID, out modelTarget);

        Quaternion trackedRotation = modelTarget.transform.rotation;
        return (model.getOutlineRotationInv() * trackedRotation).eulerAngles[1];
    }
